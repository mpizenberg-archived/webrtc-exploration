...
function initPeerConnection() {
  ...
  pc.onicecandidate = (event) =>
    sendOn(socket, "ice-candidate", event.candidate);
  pc.ontrack = (event) => {
...
function createOffer() {
  pc.createOffer().then(sendOffer).catch(logError);
}
...
function socketMessage(event_data) {
  ...
  if (data.datatype == "ice-candidate") pc.addIceCandidate(data.data);
  else if (data.datatype == "offer") receiveOffer(data.data);
  else if (data.datatype == "answer") pc.setRemoteDescription(data.data);
}
