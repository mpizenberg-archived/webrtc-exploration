async function peer(other, polite, width = 160, height = 120) {
  ...
  const camera = createCheckbox("Camera"),
    noise = createCheckbox("Noise");
  ...
  try {
    ...
    camera.onclick = () =>
      onOff(camera, () => navigator.mediaDevices.getUserMedia({ video: true }));
    noise.onclick = () => onOff(noise, whiteNoise);
    both.onclick = () => {
      dc.send(both.checked);
      dc.onmessage({ data: both.checked });
    };
    dc.onmessage = ({ data }) =>
      noise.checked == JSON.parse(data) || noise.click();

    async function onOff(button, getMedia) {
      try {
        if (button.checked) {
          if (!button.stream) button.stream = await getMedia();
          button.transceiver = pc.addTransceiver(button.stream.getTracks()[0], {
            streams: [button.stream],
          });
        } else {
    ...
    pc.ontrack = ({ streams: [stream] }) => {
      if (!stream.video) {
        stream.video = Object.assign(create(body, "video"), {
          width,
          height,
          autoplay: true,
        });
